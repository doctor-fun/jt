...
/**
 * @author fangchen
 * @date 2020/1/22 12:50 上午
 * 异步交还httpClient线程到池中，扫描整个httpClient池
 */
@Component	//交给spring容器管理
public class HttpClientClose extends Thread{
	@Autowired
	private PoolingHttpClientConnectionManager manage;
	private volatile boolean shutdown;	//开关 volatitle表示多线程可变数据,一个线程修改,其他线程立即感知到
	//线程一旦实例化，就开启
	...
	/**
	 * 通过PoolingHttpClientConnectionManager获得httpClient线程的状态，关闭5秒还没返回的httpClient方法
	 */
	@Override
	public void run() {
		try {
			...
			while(!shutdown) {
				synchronized (this) {
					...
					int lea = stats.getLeased();    //获取当前正在使用的连接数量
					int max = stats.getMax();
					//System.out.println("max/"+max+":	av/"+av+":  pend/"+pend+":   lea/"+lea);
					manage.closeExpiredConnections();//关闭超时的http连接
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
